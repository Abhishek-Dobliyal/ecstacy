...
### Changed
- Dashboard panel cache is preserved across layout toggles, and `_prepare_panel_widget` reuses cached transforms to avoid redundant group/agg/resample work.
- TableView search filter now caches string representations per-column instead of materializing the full DataFrame as strings, reducing peak memory on wide tables.
...
### Fixed
- Chart column picker OK/Cancel buttons were invisible: the button
  container had `height: 1` but Textual's `Button` has a default `tall`
  border consuming both rows. Changed to `height: auto` so buttons
  render at their natural height with visible text.
- Table search didn't update the displayed rows: `_deliver_search` set
  the filtered frame and called `add_rows` without clearing the old rows
  first, so filtered results were appended on top of the original data.
  Added `table.clear()` before rendering the new page.

## [0.10.1] - 2026-07-30

### Fixed
- Chart column picker modal could not be confirmed: the `enter` screen
  binding conflicted with `Select`'s own `enter` binding (opens the
  dropdown). Replaced the `enter` binding with explicit OK/Cancel buttons
  at the bottom of the modal. Tab to OK and press enter to confirm, or
  press escape to cancel. The chart now re-renders with the selected
  columns when OK is pressed.
