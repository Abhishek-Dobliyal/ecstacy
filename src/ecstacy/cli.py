...
from ecstacy.config.loader import load_app_config, load_dashboard, user_config_path
...
def open(  # noqa: A001 - intentional CLI verb
    ...
    _launch(
        open_spec=spec_from_target(target),
        viz=chart,
        theme=theme,
        max_rows=max_rows,
        no_splash=no_splash,
...
@app.command()
def file(
    path: str = typer.Argument(..., help="Path to csv/json/parquet/log"),
    ...
    params: dict = {"path": str(Path(path).expanduser())}
    if fmt:
        params["fmt"] = fmt
